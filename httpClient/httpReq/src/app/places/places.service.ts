...
export class PlacesService {
  ...
  loadAvailablePlaces() {  //fetching all data using get().
  ...
  loadUserPlaces() {                    //fetching user selected places using put().
    return this.fetchPlaces(
  ...
  addPlaceToUserPlaces(place: Place) {
    ...
    if(!prevPlaces.some((p)=> p.id === place.id)){  //check if place already is in array or not 
      this.userPlaces.set([...prevPlaces,place]); // Using the '...' operator copy previous placesv and  adding new selected place in array using post().
    }
  ...
  removeUserPlace(place: Place) {  //removing new selected place from array using delete().
    const prevPlaces = this.userPlaces();
